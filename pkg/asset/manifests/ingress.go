...
func (ing *Ingress) generateClusterConfig(config *types.InstallConfig) ([]byte, error) {
	...
	case aws.Name:
		...
		if config.AWS.LBType == configv1.NLB || config.IsSingleStackIpv6() || config.IsDualStack() {
			lbType = configv1.NLB
		}
...
func (ing *Ingress) generateDefaultIngressController(config *types.InstallConfig) ([]byte, error) {
	...
	case aws.Name:
		...
		// Thus, we set publish to NodePort and manage the Load Balancer ourselves.
		if config.IsSingleStackIpv6() || config.IsDualStack() {
