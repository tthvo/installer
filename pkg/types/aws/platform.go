...
type VPCSpec struct {
	...
	//
	// Subnets must contain unique IDs, must not exceed a total of 40 subnets,
	// and can include no more than 10 subnets with the IngressController role.
	//
	// Leave this field unset to have the installer create subnets in a new VPC on your behalf.
	//
	// Subnets is available in TechPreview.
	//
