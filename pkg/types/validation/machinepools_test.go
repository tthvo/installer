...
func TestValidateMachinePool(t *testing.T) {
	cases := []struct {
		...
		{
			name:     "invalid CAPI management on AWS edge",
			platform: &types.Platform{AWS: &aws.Platform{Region: "us-east-1"}},
			pool: func() *types.MachinePool {
			...
			expectedError: `edge machines cannot be managed by Cluster API`,
		},
		{
		...
		{
			name:     "invalid CAPI management on Azure",
			platform: &types.Platform{Azure: &azure.Platform{Region: "eastus"}},
			pool: func() *types.MachinePool {
				p := validMachinePool(types.MachinePoolComputeRoleName)
				p.Management = types.ClusterAPI
			...
			expectedError: `machines cannot be managed by Cluster API for platform azure`,
		},
		{
